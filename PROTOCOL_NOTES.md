...
- `sysdiagnose_2026.04.24_13-21-43-0400_iPhone-OS_iPad_22F76` and `ScreenRecording_04-24-2026 13-20-57_1.MP4` are the first Row Mode capture:
  ...
  - Row entry also writes `0x4FB0 FITNESS_WORKOUT_STATE = 8`, which collides with Isometric Test. The captured screen-state frame reports `0x3E89=0x0085`, `0x4FB0=0x08`, `0x5011 APP_CUR_SCR_ID=0x39`, and `0x5467 FITNESS_ONGOING_UI=0x0108`, but older Isometric captures can report the same screen/UI ids, so Android must not relabel state `8` as Rowing from those fields alone.
  - live Row traffic includes dense `cmd=0xB4`, `cmd=0xAA 0x92`, and `cmd=0xAA 0x93` packets. The iPad UI shows total time, total distance meters, pace per 500m, stroke rate, average pace, kcal, projected finish, and splits, but those metric byte offsets are not confirmed yet.
  - promising Row settings params from `paramInfo.csv`: `0x53A7 FITNESS_ROWING_DAMPER_RATIO_IDX` for resistance/damper and `0x53AE EP_ROW_CHAIN_GEAR` for simulated cable wear or chain gear. These are notes-only until a focused settings capture confirms writes.
