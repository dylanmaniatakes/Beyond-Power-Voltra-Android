...
- `sysdiagnose_2026.04.24_13-21-43-0400_iPhone-OS_iPad_22F76` and `ScreenRecording_04-24-2026 13-20-57_1.MP4` are the first Row Mode capture:
  ...
  - The 2026-06-08 stock Android Hermes bundle confirms the Row/Ski Cardio selector: `0x54F5 CARDIO_ACTIVITY_MODE` writes `0` for Rowing and `1` for Ski. Android writes this before Cardio entry/start so the shared row workout state reports the selected activity.
  - The same stock bundle exposes `0x5522 SKI_RESISTANCE_LEVEL` as a zero-based `0..9` selector. Android reads/parses the value and has a payload builder, but dedicated Ski resistance UI remains a follow-up until live behavior is captured.
...
- 2026-06-08 Sled Pull beta APK check from `/Users/ticnitsi/Downloads/Beyond+ (1).apk_Decompiler.com.zip`:
  - The React Native bundle is Hermes bytecode v96. Disassembling it with Hermes from React Native `0.83.9` exposed the string/function table, but searches for `Sled`, `Sled Pull`, `sledPull`, and `sled pull` found no mode label, enum, command, or translation key. The only `sled` hit was the generic Material icon name `sledding`.
  - The same APK does expose the existing Row/Ski cardio family: strings and functions for `VoltraCardioActivityMode`, `cardioActivityModeIndex`, `setCardioActivityMode`, `setSkiResistanceLevel`, `supportsSkiMode`, `ROWING_WORKOUT_TYPE = 8`, and `SKI_WORKOUT_TYPE = 9`.
  - `resolveCardioActivityFromModeIndex`, `resolveCardioActivityModeIndex`, `resolveRowingWorkoutType`, and `isSkiActivity` only branch between `rowing` and `ski`. `resolveVisibleDashboardModes` still exposes `rowing` but no Sled/Sled Pull dashboard mode.
  - Do not add an active Sled Pull BLE write from this APK alone. A beta APK containing the feature, a CodePush/OTA bundle, or a clean BLE capture from the beta flow is still required before Android should expose Sled Pull as a controllable mode.
...
- **Sled Pull beta**: no confirmed control frame yet. The 2026-06-08 decompiled Android APK only exposes Rowing/Skiing cardio evidence, so Sled Pull must stay capture-only until a beta artifact or BLE trace proves the selector, load path, and telemetry family.
